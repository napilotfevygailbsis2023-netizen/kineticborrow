...
<head>
  ...
  <style>
    *,*::before,*::after{box-sizing:border-box;margin:0;padding:0}
    ...
    .sidebar-profile-link:hover .admin-name{color:var(--gold)!important;}
...
<aside class="sidebar">
  ...
  <a href="admin_profile.php" style="display:flex;align-items:center;gap:10px;padding:14px 20px;border-bottom:1px solid var(--sidebar3);text-decoration:none;cursor:pointer;transition:background .18s;background:transparent" onmouseover="this.style.background='#2E2420'" onmouseout="this.style.background='transparent'" title="View My Profile">
    <div class="admin-avatar"><?= strtoupper(substr($admin['name'],0,1)) ?></div>
    <div class="admin-info">
      <div class="admin-name"><?= htmlspecialchars($admin['name']) ?></div>
      <div class="admin-role"><?php $rl=['superadmin'=>'Super Administrator','admin'=>'Administrator','staff'=>'Staff']; echo $rl[$admin['role']]??ucfirst($admin['role']); ?></div>
    </div>
  </a>
  <nav class="sidebar-nav">
    ...
    <a class="nav-item <?= $menu_label==='handlers'?'active':'' ?>" href="admin_handlers.php">
      <span class="nav-icon">🔧</span> Handlers
    </a>
